...
namespace Microsoft.Samples.Tools.Mdbg
{
    ...
    /// <summary>
    /// Extention containg all the commands
    /// </summary>
    public sealed class MdbgCmd : CommandBase
    {
        private static System.Resources.ResourceManager resource_manager;
        private static string last_saved_run_command;
        private static ThreadNickNames thread_nick_names;
        private static MDbgSymbolCache symbol_cache;
        private static ArrayList events;
        private static Dictionary<string, bool> loaded_extensions;

        /// <summary>
        /// Initialize this extension. 
        /// We can only initialize global (non-process specific) state right now since we may not have an active process. 
        /// However, we can subscribe to the ProcessAdded event to register per-process state.
        /// </summary>
        public static void Initialize()
        {
            MdbgCmd.resource_manager = new System.Resources.ResourceManager("mdbgCommands", Assembly.GetExecutingAssembly());
            MDbgAttributeDefinedCommand.AddCommandsFromType(Shell.Commands, typeof(MdbgCmd));

            MdbgCmd.last_saved_run_command = null;
            MdbgCmd.thread_nick_names = new ThreadNickNames();
            MdbgCmd.symbol_cache = new MDbgSymbolCache();
            MdbgCmd.events = new ArrayList();
            MdbgCmd.loaded_extensions = new Dictionary<string, bool>();
        ...
        public static void RunCmd(string arguments)
        {
            ...
            if (arguments.Length == 0 && last_saved_run_command != null)
            {
                ...
                arguments = last_saved_run_command;
            }
            ...
            p.Go().WaitOne();
            last_saved_run_command = arguments;
        ...
        private static void InternalWhereCommand(MDbgThread thread, int depth, bool verboseOutput)
        {
            ...
            WriteOutput("Thread [#:" + thread_nick_names.GetThreadName(thread) + "]");
        ...
        class DefaultBreakpointParser : IBreakpointParser
        {
            ...
            ISequencePointResolver IBreakpointParser.ParseFunctionBreakpoint(string arguments)
            {
                ...
                if (symNum.Length > 0)
                {
                    ...
                    MdbgSymbol symbol = symbol_cache.Retrieve(intSymNum);
        ...
        public static void ListAllBreakpoints()
        {
        ...
        public static void ThreadCmd(string arguments)
        {
            ...
            if (ap.Count == 0)
            {
                ...
                {
                    thread_nick_names.SetThreadNickName(ap.GetOption(nickNameStr).AsString, Debugger.Processes.Active.Threads.Active);
                }
                else
                {
                    ...
                    foreach (MDbgThread t in p.Threads)
                    {
                        ...
                        WriteOutput(string.Format("th #:{0} (ID:{1}){2}",
                                                  thread_nick_names.GetThreadName(t),
                                                  t.Id,
            ...
            {
                MDbgThread t = thread_nick_names.GetThreadByNickName(ap.AsString(0));
        ...
        public static void LoadCmd(string arguments)
        {
            ...
            if (loaded_extensions.ContainsKey(extName) && loaded_extensions[extName])
            {
                ...
                loaded_extensions[extName] = false;    // Make sure that we mark the extension as unloaded
            }

            ExecuteExtensionMethod(asext, "LoadExtension");

            if (loaded_extensions.ContainsKey(extName))
            {
                // We have loaded this extension before, just update the status
                Debug.Assert(!loaded_extensions[extName], "Extension already marked active");
                loaded_extensions[extName] = true;
            }
            else
            {
                // This is the first time this extension has loaded, create a new entry
                loaded_extensions.Add(extName, true);
            }
        }
        ...
        public static void UnloadCmd(string arguments)
        {
            ...
            else if (!loaded_extensions.ContainsKey(extName) || !loaded_extensions[extName])
            {
            ...
            try
            {
                ...
                loaded_extensions[extName] = false;
            }
            ...
            else
            {
                ...
                foreach (Type t in m.Importer.DefinedTypes)
                {
                    foreach (MethodInfo mi in t.GetMethods())
                    {
                        ...
                        if (shouldPrint)
                        {
                            int idx = symbol_cache.Add(new MdbgSymbol(m.Number, t.Name, mi.Name, 0));
        ...
        public static void WhenCmd(string arguments)
        {
            ...
            if (ap.Count == 0)
            {
                ...
                foreach (ExecuteCmdAction a in events)
                {
        ...
        public static void Step(Action PreBreak, Action<LocationState> PostBreak)
        {
            Debugger.Processes.Active.StepInto(false).WaitOne();
        ...
        private struct VariableLogInfo
        {
            ...
            public VariableLogInfo(int lineNumber, string filePath, string variableName, string variableValue)
            {
